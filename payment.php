  ...
  <div class="payment">
    ...
    <div class="form">
	  ...
	  <div class="card space icon-relative">
        ...
        <input type="text" value="<?=$_SESSION['price']?>" class="input" placeholder="Name On Card">
	...
	{
		extract($_REQUEST);
		$abc=select("SELECT MAX(id) FROM `booking`");
		while($a=mysqli_fetch_array($abc))
		{
			$id=$a[0];
			$amount=$_SESSION['price'];
			$n=iud("INSERT INTO `payment`(`booking_id`, `amount`) VALUES ('$id','$amount')");
			if($n==1)
			{
				unset($_SESSION['price']);
				echo"<script>alert('Successful We Will Contact You Very Soon');
